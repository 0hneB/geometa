...
├── docs/                             # Focused migration and design notes
├── scripts/
...
Development-mode builds can override the production endpoints with `VITE_GEOMETA_API_URL` and `VITE_GEOMETA_SITE_URL`. Use the frontend and API URLs printed by Process Compose. Either override switches the userscript to its local-development namespace; `VITE_GEOMETA_API_URL` also isolates cached map lookups from the production cache.
...
- [Multiple metas per location](docs/multi-meta-locations.md)
