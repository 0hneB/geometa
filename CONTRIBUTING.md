...
Documentation pages are MDsveX routes under
`apps/frontend/src/routes/(public)/docs` and run with the normal frontend. Use
`npm run dev`, `npm run test:docs`, and `npm run build` from `apps/frontend`
when editing them. The Scalar API reference is generated by the API at
`/docs/api`, using the OpenAPI document served at `/api/docs/json`.
